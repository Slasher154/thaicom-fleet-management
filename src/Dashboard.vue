<template>
    <div>
        <h1>Dashboard</h1>
        <!--<button class="btn btn-primary" @click="transport">Change Car Position</button>-->
        <hr>
        <h4>Speed Limit: {{ speedLimit }} km/h</h4>
        <div class="row">
        ...
        methods: {
            transport() {
                this.cars.forEach((car) => {
                    car.position.lat += Math.random() / 10;
                    car.position.lng += Math.random() / 10;
                })
            },
